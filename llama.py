            ...
            for i in range(num_users):
                ...
                if freivalds:
                    try:
                        ...
                        # Ensure dtype consistency - convert to float32 for communication
                        q_local_f32 = q_local.float()
                        # 1) send Q to worker
                        torch.distributed.isend(q_local_f32, 1 + i)
                        # 2) compute and send y = Q @ r
                        y = (q_local_f32 * r.unsqueeze(0).unsqueeze(2)).sum(dim=-1)
                        ...
                        q_local_f32 = q_new_cpu[i * batch_per_user:(i + 1) * batch_per_user].float()
                        torch.distributed.isend(q_local_f32, 1 + i)
                else:
                    q_local_f32 = q_new_cpu[i * batch_per_user:(i + 1) * batch_per_user].float()
                    torch.distributed.isend(q_local_f32, 1 + i)
